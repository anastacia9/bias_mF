# Compute a median table of CAI codon table values from genomic sequence files in input path and based on genes specified in an input array.
...
#         [1] numpy array of reference set gene names
#
# output: [0] dictionary of median relative adaptiveness values across strains
#
...
def codontable_Carbone(path, array):
 ...
 # Specify empty array of arrays of strain codon count sums for median-ing later on.
 num_files = len(os.listdir(path))
 dict_w_aggregate = [None] * num_files

 # One file at a time . . .
 for file in range(0, num_files):
  # compute reference set codon counts . . .
  codon_counts = numpy.zeros(61)
  ...
  for seq in seqs:
   ...
   # adding sequence codon counts together.
   codon_counts = codon_counts + numpy.array(list(dict_codons.values()))

  # Divide sum of codon i values by sum of values of most prevalent synonymous codon.
  dict_codon_counts = dict(zip(amino_flatten, codon_counts))

  dict_w = {}
  for i in amino:
   frequency = numpy.asarray([dict_codon_counts[x] for x in i])
