...
#
#strains = ['273614N', '378604X', 'BC187', 'DBVPG1106', 'DBVPG1373', 'DBVPG6765', 'L_1374', 'NCYC361', 'README.tx', 'SK1', 'UWOPS05_217_3', 'UWOPS05_227_2', 'UWOPS83_787_3', 'UWOPS87_2421', 'Y12', 'Y55', 'YJM975', 'YJM978', 'YJM981', 'YPS128', 'YPS606', 'YS2', 'YS9']
#calcG_function = 'struc_frac'
#length_csvfile = 'projectfiles/data/yeast_UTR5_CDS_UTR3/UTR5_CDS_UTR3_lengths_1461.csv'
#domain_csvfile = 'projectfiles/data/yeast_domain/domains_total_Pfam.csv'
#import mRNA_sectionG_working
#for strain in strains:
#
#strain = strains[0]
#RNAfold_outputfile = 'projectfiles/data/yeast_RNAfold/'+strain+'_fold.txt'
#RNAeval_outputfile = 'projectfiles/data/yeast_RNAeval/'+strain+'_eval.txt'
#a = mRNA_sectionG_working.mRNA_sectionG(calcG_function, length_csvfile, domain_csvfile, RNAfold_outputfile, RNAeval_outputfile)
...
def mRNA_sectionG(calcG_function, length_csvfile, domain_csvfile, RNAfold_outputfile, RNAeval_outputfile):
 ...
 for i in range(0, num_genes):                                         #
     ...
     UT5_deltaG[i]  = getG( deltaGs[ (len5+int(lenc/4)) : (len5+int(lenc/4)+40) ] )                            # define deltaG of 5'UTR
     CDS_deltaG[i]  = getG( deltaGs[ (len5+int(lenc/2)) : (len5+int(lenc/2)+40) ] )                   # define deltaG of CDS
     UT3_deltaG[i]  = getG( deltaGs[ (len5+3*int(lenc/4)) : (len5+3*int(lenc/4)+40) ] )                         # define deltaG of 3'UTR
     ...
     if len5 >= 19:                                                    # we have a section overlap problem if the 5'UTR is less than 19 nucleotides long
       cap_deltaG[i] = getG( deltaGs[ (len5+int(lenc/2)) : (len5+int(lenc/2)+9) ] )      # define deltaG of cap region
     ...
     if gene_names[i] in dgene_names:                                  #
       ...
       vals        = domain_info.to_numpy().flatten()                  # flatten array of domain coordinates
       num_vals    = len(vals)                                         #
