...
response <- read.csv("C:/Users/anastacia/Desktop/n_PPR_1447_d.csv")
dataset  <- melt(response, id.vars = "gene", variable.name = "strain", value.name = "PPR")
dataset$logPPR <- log(dataset$PPR)
```
##### Read in tAI Data
```{r Ftai}
explanatory0 <- read.csv('C:/Users/anastacia/Desktop/tAI_1447_YRC_diff_d.csv')
...
##### Read in deltaG Data
```{r FdelG}
explanatory1 <- read.csv('C:/Users/anastacia/Desktop/ensemble_fe_1447_30C.csv')
...
``` {r F2}
lmm00 <- lmer(logPPR ~ (0+tAI|gene) + (0+ensembleG|gene) + (0+tAI:ensembleG|gene) + (1|gene), data=dataset, REML = FALSE, control = lmerControl(optimizer="Nelder_Mead", optCtrl=list(maxfun=1e5)))
anova(lmm0, lmm00)
```
