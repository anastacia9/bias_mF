...
dataset$tAI = explanatory3$tAI #multiply resulting slope by 10 to get units of 1/(Mcal per mol)
```

##### Summary Results of Mixed Model
``` {r F1}
lmm0 <- lmer(sqrtPPR ~ tAI + UTR5 + CDS + UTR3 + tAI:UTR5 + tAI:CDS + tAI:UTR3 +(0+tAI|gene) +(0+UTR3|gene) + (1|gene), data=dataset, REML = FALSE, control = lmerControl(optimizer="Nelder_Mead", optCtrl=list(maxfun=1e5)))
summary(lmm0)
```
