...
### Figure 2B - analysis of mF on full set of genes
...
``` {r A1}
lmm1 <- lmer(logPPR ~ optimalG + (1+optimalG|gene), data=dataset, REML = FALSE, control = lmerControl(optimizer="bobyqa", optCtrl=list(maxfun=1e5)))
summary(lmm1)
```
...
dataset$bp_prob <- explanatory0$bp_prob
```
...
### Figure 2D - With ensemble deltaG, look at subsets of genes and medianPA as a random effect
...
##### Summary Results of Mixed Model
``` {r F1_partial}
lmm0 <- lmer(logPPR ~ ensembleG + (1+ensembleG|gene), data=dataset, REML = FALSE, control = lmerControl(optimizer="Nelder_Mead", optCtrl=list(maxfun=1e5)))
summary(lmm0)
```
...
##### Fixed Effects Slope Significance Results
``` {r F2_partial}
lmm00 <- lmer(logPPR ~ (1+ensembleG|gene), data=dataset, REML = FALSE, control = lmerControl(optimizer="Nelder_Mead", optCtrl=list(maxfun=1e5)))
anova(lmm0, lmm00)
```
...
explanatory0 <- melt(explanatory0, id.vars = "gene", variable.name = "strain", value.name = "ensembleG")
dataset$ensembleG = explanatory0$ensembleG/1000 #divided by 1000 -> deltaG in Mcal/mol -> don't need to scale slope
```
...
##### Summary Results of Mixed Model
``` {r F1medpa}
lmm0 <- lmer(logPPR ~ ensembleG + (1+ensembleG|gene) + (0+ensembleG|medianPA), data=dataset, REML = FALSE, control = lmerControl(optimizer="bobyqa", optCtrl=list(maxfun=1e5)))
summary(lmm0)
```
...
##### Fixed Effects Slope Significance Results
``` {r F2medpa}
lmm00 <- lmer(logPPR ~ (1+ensembleG|gene) + (0+ensembleG|medianPA), data=dataset, REML = FALSE, control = lmerControl(optimizer="bobyqa", optCtrl=list(maxfun=1e5)))
anova(lmm0, lmm00)
```
