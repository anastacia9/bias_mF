...
### Figure 2A - Analysis of Codon Bias on the Full Set of Genes
...
tiff(file="cai_logppr_res_fit.tiff", width = 12, height = 12, units = 'cm', res = 300)
scatter.smooth(fitted(lmm1), res, col='black', span=0.1, xlab='Fitted logPPR values', ylab='Residuals')
dev.off()
...
``` {r tAI1}
lmm5 <- lmer(logPPR ~ tAI + (1+tAI|gene), data=dataset, REML = FALSE, control = lmerControl(optimizer="Nelder_Mead", optCtrl=list(maxfun=1e5)))
summary(lmm5)
```
...
##### Summary Results of Model
``` {r StAIt1}
lmm1 <- lmer(logPPR ~ tAI + (1+tAI|gene), data=dataset, REML = FALSE, control = lmerControl(optimizer="bobyqa", optCtrl=list(maxfun=1e5)))
summary(lmm1)
```

##### Fixed Effects Slope Significance Results
``` {r tAIt2}
lmm2 <- lmer(logPPR ~ (1+tAI|gene), data=dataset, REML = FALSE, control = lmerControl(optimizer="bobyqa", optCtrl=list(maxfun=1e5)))
...
##### Summary Results of Model
``` {r tAIhighpa1}
lmm3 <- lmer(logPPR ~ tAI + (1+tAI|gene), data=dataset, REML = FALSE, control = lmerControl(optimizer="bobyqa", optCtrl=list(maxfun=1e5)))
summary(lmm3)
```
...
##### Fixed Effects Slope Significance Results
``` {r tAIhighpa2}
lmm4 <- lmer(logPPR ~ (1+tAI|gene), data=dataset, REML = FALSE, control = lmerControl(optimizer="bobyqa", optCtrl=list(maxfun=1e5)))
...
##### Summary Results of Model
``` {r tAImedpa1}
lmm7 <- lmer(logPPR ~ tAI + (0+tAI|gene) + (1+tAI|medianPA), data=dataset, REML = FALSE, control = lmerControl(optimizer="Nelder_Mead", optCtrl=list(maxfun=1e5)))
summary(lmm7)
```

##### Fixed Effects Slope Significance Results
``` {r tAImedpa2}
lmm8 <- lmer(logPPR ~ (0+tAI|gene) + (1+tAI|medianPA), data=dataset, REML = FALSE, control = lmerControl(optimizer="bobyqa", optCtrl=list(maxfun=1e5)))
tai1620m<-anova(lmm7, lmm8)
tai1620m
```
