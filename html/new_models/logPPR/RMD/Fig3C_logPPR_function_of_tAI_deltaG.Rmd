---
title: "logPPR as a function of tAI, ensemble deltaG, and their interaction"
date: "3/15/2023"
...
#### logPPR ~ tAI + deltaG + tAI:deltaG {.tabset .tabset-dropdown}
##### Menu
...
``` {r F1}
lmm0 <- lmer(logPPR ~ tAI + ensembleG + tAI:ensembleG + (0+ensembleG|gene) + (0+tAI:ensembleG|gene) + (0+tAI|medianPA) + (0+tAI:ensembleG|medianPA) + (1|medianPA), data=dataset, REML = FALSE, control = lmerControl(optimizer="bobyqa", optCtrl=list(maxfun=1e5)))
summary(lmm0)
```
...
``` {r F2}
lmm00 <- lmer(logPPR ~ (0+ensembleG|gene) + (0+tAI:ensembleG|gene) + (0+tAI|medianPA) + (0+tAI:ensembleG|medianPA) + (1|medianPA), data=dataset, REML = FALSE, control = lmerControl(optimizer="Nelder_Mead", optCtrl=list(maxfun=1e5)))
...
##### Fixed Effects Slope Significance Results compared to model without interaction term
``` {r F2d}
lmm00 <- lmer(logPPR ~ tAI + ensembleG + (0+ensembleG|gene) + (0+tAI:ensembleG|gene) + (0+tAI|medianPA) + (0+tAI:ensembleG|medianPA) + (1|medianPA), data=dataset, REML = FALSE, control = lmerControl(optimizer="Nelder_Mead", optCtrl=list(maxfun=1e5)))
fullcomp<-anova(lmm0, lmm00)
fullcomp
```
