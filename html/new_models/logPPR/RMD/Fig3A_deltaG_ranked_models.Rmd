---
title: "tAI vs. logPPR analyses for genes ranked by low and high deltaG"
...
``` {r tAI_low_G1}
lmm0 <- lmer(logPPR ~ tAI_low_G + (1+tAI_low_G|gene) + (1+tAI_low_G|medianPA), data=dataset, REML = FALSE, control = lmerControl(optimizer="bobyqa", optCtrl=list(maxfun=1e5)))
summary(lmm0)
```

##### Fixed Effects Slope Significance Results
``` {r tAI_low_G2}
lmm1 <- lmer(logPPR ~ (1+tAI_low_G|gene) + (1+tAI_low_G|medianPA), data=dataset, REML = FALSE, control = lmerControl(optimizer="bobyqa", optCtrl=list(maxfun=1e5)))
...
##### Summary Results of Mixed Model
``` {r tAI_high_G1}
lmm0 <- lmer(logPPR ~ tAI_high_G + (1+tAI_high_G|gene) + (1+tAI_high_G|medianPA), data=dataset, REML = FALSE, control = lmerControl(optimizer="bobyqa", optCtrl=list(maxfun=1e5)))
summary(lmm0)
```

##### Fixed Effects Slope Significance Results
``` {r tAI_high_G2}
lmm1 <- lmer(logPPR ~ (1+tAI_high_G|gene) + (1+tAI_high_G|medianPA), data=dataset, REML = FALSE, control = lmerControl(optimizer="bobyqa", optCtrl=list(maxfun=1e5)))
