...
``` {r F1}
lmm0 <- lmer(logPPR ~ tAI + cap + upA + dwA + dwS + tAI:cap + tAI:upA + tAI:dwA + tAI:dwS +(0+tAI|gene)+(0+cap|gene)+(0+dwA|gene) + (1|gene) + (0+tAI|medianPA)+(0+cap|medianPA)+(0+upA|medianPA)+(0+dwS|medianPA) + (1|medianPA), data=dataset, REML = FALSE, control = lmerControl(optimizer="bobyqa", optCtrl=list(maxfun=1e5)))
summary(lmm0)
```
