---
title: "sqrtPPR as a function of tAI, 5'UTR, CDS, 3'UTR deltaG, and their interactions"
date: "3/15/2023"
...
#### sqrtPPR ~ tAI + 5'UTR deltaG + CDS deltaG + 3'UTR deltaG + all pairwise interactions{.tabset .tabset-dropdown}
##### Menu
...
dataset$CDS = explanatory1$CDS/1000/1000
```
...
``` {r F1}
lmm0 <- lmer(sqrtPPR ~ tAI + UTR5 + CDS + UTR3 + tAI:UTR5 + tAI:CDS + tAI:UTR3 + (0+UTR5|gene) + (0+CDS|gene) + (1|gene) + (0+tAI|medianPA) + (0+UTR5|medianPA) + (0+CDS|medianPA) + (0+UTR3|medianPA) + (1|medianPA), data=dataset, REML = FALSE, control = lmerControl(optimizer="bobyqa", optCtrl=list(maxfun=1e5)))
summary(lmm0)
```
