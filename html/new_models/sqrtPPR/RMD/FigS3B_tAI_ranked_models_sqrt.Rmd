---
title: "deltaG vs. sqrtPPR analyses for genes ranked by low, medium, and high tAI"
...
dataset$deltaG_low_tAI <- explanatory0$deltaG_low_tAI/1000
```
...
``` {r deltaG_low_tAI1}
lmm0 <- lmer(sqrtPPR ~ deltaG_low_tAI + (1|medianPA), data=dataset, REML = FALSE, control = lmerControl(optimizer="Nelder_Mead", optCtrl=list(maxfun=1e5)))
summary(lmm0)
```

##### Fixed Effects Slope Significance Results
``` {r deltaG_low_tAI2}
lmm1 <- lmer(sqrtPPR ~  (1|medianPA), data=dataset, REML = FALSE, control = lmerControl(optimizer="Nelder_Mead", optCtrl=list(maxfun=1e5)))
anova(lmm0, lmm1)
```
...
dataset$deltaG_high_tAI <- explanatory0$deltaG_high_tAI/1000
```
...
##### Summary Results of Mixed Model
``` {r deltaG_high_tAI1}
lmm0 <- lmer(sqrtPPR ~ deltaG_high_tAI + (1+deltaG_high_tAI|medianPA), data=dataset, REML = FALSE, control = lmerControl(optimizer="Nelder_Mead", optCtrl=list(maxfun=1e5)))
summary(lmm0)
```

##### Fixed Effects Slope Significance Results
``` {r deltaG_high_tAI2}
lmm1 <- lmer(sqrtPPR ~ (1+deltaG_high_tAI|medianPA), data=dataset, REML = FALSE, control = lmerControl(optimizer="Nelder_Mead", optCtrl=list(maxfun=1e5)))
anova(lmm0, lmm1)
```
