...
``` {r F1}
lmm0 <- lmer(sqrtPPR ~ tAI + onefour + tAI:onefour + (0+tAI|gene) + (0+onefour|gene) + (0+tAI|medianPA) + (0+onefour|medianPA) + (1|medianPA), data=dataset, REML = FALSE, control = lmerControl(optimizer="bobyqa", optCtrl=list(maxfun=1e5)))
summary(lmm0)
```
...
##### Summary Results of Mixed Model
``` {r F12}
lmm0 <- lmer(sqrtPPR ~ tAI + onehalf + tAI:onehalf + (0+tAI|gene) + (0+tAI|medianPA) + (0+onehalf|medianPA) + (1|medianPA), data=dataset, REML = FALSE, control = lmerControl(optimizer="Nelder_Mead", optCtrl=list(maxfun=1e5)))
summary(lmm0)
```
##### 95% Confidence Intervals
``` {r F1confint2}
...
##### Summary Results of Mixed Model
``` {r F13}
lmm0 <- lmer(sqrtPPR ~ tAI + threefour + tAI:threefour + (0+tAI:threefour|gene) + (1|gene) + (0+tAI|medianPA) + (1|medianPA), data=dataset, REML = FALSE, control = lmerControl(optimizer="Nelder_Mead", optCtrl=list(maxfun=1e5)))
summary(lmm0)
```
